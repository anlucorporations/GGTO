...
| D-36 | **Respaldo a demanda del supervisor** (P9). La rutina y la rotación se fijaron después en D-42 (10 versiones `.bak`) y D-49 (copia fechada al cierre). **Superado en lo relativo al versionado por D-71:** el supuesto de «repositorios privados» era falso (son públicos), así que el CSV, los PDF de despacho y el `.xlsm` **no se versionan** y se purgaron del historial. |
...
| D-58 | **Datos personales:** se mantiene D-36 y se documenta la finalidad y los controles en la ficha de tratamiento (`entornos_globales.md` §12). El log de la aplicación rota por tamaño: **5 MB y 5 archivos**, sin datos personales (H-N-31). **Matizado por D-71:** los repositorios **no** son privados, así que los archivos con datos de abonados dejaron de versionarse. |
...
| D-69 | **La dirección no es campo obligatorio de la ingesta:** `estructura.json` declaraba `direccion` (col. 34) como obligatoria, pero el archivo real del 12/09/2026 trae **4 de 51** filas sin dirección y CU-08 exige insertar las 51. La columna deja de declararse obligatoria; esas filas **entran igualmente** y quedan en la **cola de direcciones sin sector** (CU-09, RN-04). Es obligatoria en el **maestro** (se completa después), no en el contrato del CSV. *(Detectado en las pruebas de contrato de la Fase 4.)* |
| D-70 | **El bloque INGESTA (CU-08) se aloja en la pestaña PANEL:** el fuente lo define como sección propia, pero las pestañas son **7** (RF-01) y CU-08 lo llama «bloque»; se muestra dentro del PANEL —la pantalla principal— y lo pueden ejecutar **operador y supervisor**, como fija la matriz de §2.1 (se corrige la contradicción de CU-01 CA-12). *(Detectado en las pruebas E2E de la Fase 4: el bloque no tenía punto de entrada.)* |
| D-71 | **Los repositorios son públicos y los datos operativos reales no se versionan:** se mantiene la visibilidad **pública** de GitHub y GitLab (decisión del usuario, 14/09/2026) y se **purgan** del control de versiones y del historial los archivos con datos de abonados —el CSV diario, `alta_manual.csv`, los PDF de despacho y el `.xlsm`—, que siguen en disco fuera del repositorio. En su lugar se versiona una **muestra anonimizada** que conserva la estructura, el filtro por central, el `estatus` y las palabras clave de RN-03 y reproduce el reparto documentado. **Sustituye al supuesto de «repositorios privados» de D-36/D-58**, que era falso: lo detectó la reauditoría del documento técnico (RN-01) y lo verificó el propio análisis contra las APIs de ambas plataformas. |
| D-72 | **Averías concentradas (RF-26, CU-20):** el conteo es el **total de casos abiertos del sector** en la semana operativa, **sin excluir ninguna clase** (la construcción, `CNS`, cuenta igual; su desglose se informa aparte) y el **umbral admite 1** (mínimo entero ≥ 1, como fija CU-20 CA-3). El corte semanal usa **`ingreso`** (fecha de ingesta) y, **solo si falta**, `fecha_reporte`. *(Corrige RN-21 de la reauditoría: el código excluía `CNS` y el formulario no admitía el umbral 1, ambos sin decisión que lo respaldara.)* |
| D-73 | **El asiento de entrega del despacho no lleva el nombre del receptor:** `incidencias.log` se declara **sin datos personales** (D-58), así que la línea registra fecha, cuadrilla, número de copia y operador, y el nombre del receptor queda **solo en la hoja impresa** y en el control documental de la sesión. *(Corrige RN-05: el asiento escribía el receptor en texto libre.)* |
| D-74 | **Riesgos de operación aceptados y declarados:** el asiento del log se escribe releyendo y reescribiendo el archivo **sin el protocolo de escritura verificada** de D-42 (RN-19); el respaldo del cierre sale del equipo por un **acto manual del supervisor, sin verificación de salida**, y el **RTO de 1 hora no es alcanzable sin equipo sustituto** si se pierde el PC (RN-11). Se aceptan con esta constancia; su endurecimiento queda para una revisión posterior con el puesto delante. |
| D-75 | **Ajustes de contrato y de permisos de la reauditoría (actores y matriz):** (a) la **col. 30 `cliente_notificado`** del CSV **no se persiste** y se añade a `no_se_persisten` (`[18, 30, 53, 80]`); (b) la **cola de direcciones sin sector (CU-09) la resuelve el supervisor** en el MVP, porque la «propuesta del operador con aprobación del supervisor» de D-60 **no está implementada**: el operador no tiene la acción `sector.cola`; y (c) la matriz de §3.4 del documento técnico se alinea con la matriz única de `casos_uso.md` §2.1 y con `nucleo.js`: **el operador no ve MONITOREO, GRAFICOS ni reportes**. Si se implementa la propuesta de D-60, hay que devolver la acción al operador con su flujo de aprobación. |
| D-76 | **REPORTES (CU-19, CU-20) no es una pestaña propia: se renderiza en la sub-pestaña «REPORTES y seguimiento» de MONITOREO.** RF-01 fija siete pestañas y MONITOREO ya exige `monitoreo.ver` (supervisor), la misma audiencia que `concentradas.ver`; la pestaña aloja dos sub-pestañas (TABLERO y REPORTES) con el mismo patrón con que CONFIGURACION aloja RESPALDO y ENTORNO (D-70). Cierra el hallazgo **R2-03** de la reauditoría: `reportes.js` no tenía ninguna ruta que lo renderizara en la aplicación. |
| D-77 | **Lanzador de la jornada `GGTO.bat` + `GGTO.ps1`:** doble clic prepara el puesto (**despliegue**) y arranca la jornada. El despliegue es idempotente —comprueba la aplicación (16 `.js` y 2 librerías), el runtime, crea `datos`/`respaldo`/`despachos` si faltan, siembra desde `nucleo.js` los archivos de trabajo que falten y valida los existentes (incluido el contrato **v2** de `estructura.json`)— y la jornada informa **dónde está el CSV del día**, imprime la rutina diaria y delega el servidor en `servir-ggto.ps1`. **El lanzador NO hace copias de seguridad** (la copia de cierre verificada sigue siendo la de D-49) y **no escribe en los archivos de trabajo**. |
| D-78 | **Los CSV de ingesta viven en `C:\GGTO\datos` y solo se buscan allí:** el lanzador lista los `.csv` de esa carpeta (señalando el del día, `detalle_averias_gpon DD_MM_YYYY.csv`) y **no** busca en el proyecto, ni en Descargas, ni en el Escritorio, porque son datos de abonados y no deben dispersarse por el equipo (D-71). La carpeta de datos es la que la página ya tiene autorizada, así que el operador elige el archivo ahí mismo con el selector de INGESTA; el almacén ignora cualquier archivo que no sea uno de los 10 de trabajo, de modo que el `.csv` no interfiere con la carga ni entra en la copia de cierre. |
| D-79 | **La INGESTA elige el CSV desde la propia página:** el bloque lista los `.csv` de la carpeta de datos autorizada (nombre, tamaño y fecha, **preseleccionando el del día**) y lee el elegido por la API de archivos, sin pasar por el explorador de Windows. El selector del navegador se conserva como alternativa para un archivo que todavía no está en la carpeta. Solo se listan y se leen nombres, tamaños y texto: **no** se mueve, renombra ni borra ningún archivo de la carpeta. |
...
| RF-25 | Reportes: el despacho diario se emite en pantalla y en PDF (por cuadrilla); el seguimiento semanal es estadístico — ingreso del día vs. reparadas del día con la línea del pendiente al cierre, agrupado por semana del año (Sem 1 a Sem 36) con selector de semana. Se emite en la sub-pestaña **REPORTES y seguimiento** de MONITOREO (D-76). | C6 | L2; D-34, D-76 |
| RF-26 | Seguimiento de casos especiales (EMP o REF abiertos, D-33) y de **averías concentradas**: 3 o más casos abiertos del mismo sector en la semana operativa, con umbral editable en CONFIGURACION. Se muestra en la sub-pestaña **REPORTES y seguimiento** de MONITOREO (D-76). | C6 | L2, L13; D-25, D-33, D-76 |
...
| RNF-05 | Impresión: el PDF de despacho por cuadrilla cabe en carta horizontal. **Volumen de referencia: 60 casos en una cuadrilla** (el mayor usado en las pruebas de C4, `pruebas_c4.mjs`); un volumen mayor debe confirmarse en el puesto. | Impresión/visualización del PDF con **60 casos** en una cuadrilla y comprobación de que la tabla ocupa exactamente el área imprimible. |
...
| RNF-13 | Accesibilidad: toda la operación es posible con teclado (Tab, Enter y flechas en la tabla), el foco es visible, cada campo tiene `label` asociado, el contraste es de al menos 4,5:1 y cada gráfico tiene tabla o texto alternativo equivalente (H-05, D-40). **Módulos que implementan la alternativa textual: `metricas.js` (tablas del MONITOREO) y `graficos.js` (tabla equivalente junto a cada gráfico).** | Recorrido completo de las 7 pestañas solo con teclado y verificación de contraste con herramienta automática. *La comprobación efectiva de contraste y teclado exige la aplicación en el puesto: queda como riesgo residual de la Fase 4.* |
...
> **Categorías de ISO 25010 sin RNF propio (declarado, RN-13 de la reauditoría).** El corpus cubre
> funcionalidad, usabilidad, desempeño, fiabilidad, seguridad y portabilidad, pero **cuatro categorías
> quedan sin requisito propio**: **mantenibilidad** (no hay criterio de legibilidad ni de cobertura),
> **disponibilidad y continuidad** (el puesto es único; D-74 acepta el riesgo), **observabilidad formal**
> (el log existe, pero sin formato ni niveles de severidad normados) y **cumplimiento legal/privacidad**
> (la ficha de tratamiento está pendiente de validación por el área legal, §12 de `entornos_globales.md`).
> Se declaran aquí para que no se den por cubiertas: no bloquean el cierre de la Fase 2, pero deben
> resolverse o aceptarse por escrito antes de ampliar el uso del sistema.
...
| RT-11 | El metadata de git (`.git`) vive en disco local, fuera de la unidad sincronizada: Google Drive corrompió `.git\refs` con archivos `desktop.ini` (D-22). Tras la mudanza de D-51 el repositorio de trabajo es **`C:\GGTO\proyecto\.git`** y el antiguo **`C:\GGTO\git\GGTO-v1.git`** se conserva solo como copia histórica. |
