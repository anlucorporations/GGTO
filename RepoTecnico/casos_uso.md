...
- **Fuentes normativas (leídas completas, no modificadas):**
  ...
  `RepoTecnico/requerimientos.md` (29 RF, **16 RNF**, 11 RT, 8 RN, decisiones **D-01 a D-79**, ambigüedades A-01 a A-18 **todas cerradas**),
...
| Resolver la cola de direcciones sin sector (CU-09) | **Prohibida en el MVP (D-75):** D-60 preveía que el operador **proponga** el sector y el supervisor lo apruebe, pero ese flujo de propuesta **no está implementado**, así que la acción `sector.cola` es del supervisor | Permitida (resuelve la cola; es la única sesión que tiene la acción, D-59/D-60/**D-75**) |
...
12. **Dado** un técnico activo con P00 `12345` cuyo `cuadrillas.id` es `C1`, **Cuando** inicia sesión, **Entonces** el sistema presenta las **7 pestañas** de RF-01 —PANEL, MONITOREO, GRAFICOS, CASOS, DESPACHO, CONFIGURACION y GESTION—, habilita **CASOS** (solo los casos de `C1`) y **PANEL**, y **no** habilita MONITOREO, GRAFICOS, DESPACHO, CONFIGURACION ni GESTION; los bloques **INGESTA** (CU-08) y **RESPALDO** (CU-21) **no son pestañas** y se rigen por la matriz de §2.1: **RESPALDO queda bloqueado** para el operador y **la INGESTA sí se le muestra** —va dentro del PANEL y §2.1 la permite a los dos roles (corrección de la contradicción detectada en las pruebas de Fase 4)—. [RF-01, D-35, RNF-12, **D-70**]
...
**Trazabilidad:** RF-16, RF-17, RF-18, RF-19; RN-01, RN-02, RN-03, RN-04; RNF-02, RNF-04, RNF-06, RNF-08, RNF-09, RNF-10, **RNF-12, RNF-14, RNF-15**; RT-02, RT-03, RT-04, RT-07, RT-08; D-05, D-06, D-10, D-12, D-17, D-21, D-26, **D-35, D-38, D-41, D-42, D-44, D-46, D-52, D-53, D-54, D-56, D-59, D-78, D-79**; H-03, H-08, H-09, H-14, H-15, H-24, H-27.
...
1. El operador abre la pestaña **PANEL**, que aloja el bloque **INGESTA** (no es una pestaña propia: las pestañas son 7; **D-70**). El bloque le ofrece **los archivos `.csv` que ya están en `C:\GGTO\datos`** —la carpeta de datos autorizada, única donde se buscan (D-78)— con su tamaño y su fecha, **preseleccionando el del día** (`detalle_averias_gpon DD_MM_AAAA.csv`), y lo revisa con *Revisar el archivo elegido* sin salir de la página (**D-79**). Alternativamente puede pulsar el selector y elegir un archivo de otro sitio (un pendrive, la red), que es el camino para lo que todavía no está en la carpeta.
...
> **Alcance en el MVP (D-75).** El flujo de D-60 —el operador **propone** el sector y el supervisor **aprueba**— **no está implementado**: en el MVP la cola la resuelve **el supervisor**, que es la única sesión con la acción `sector.cola`. Mientras no exista el estado `PENDIENTE DE APROBACIÓN`, este caso de uso se ejecuta con el supervisor como actor efectivo; el operador conserva la consulta del caso.
...
- **Punto de entrada:** el bloque **REPORTES** se renderiza en la **sub-pestaña «REPORTES y seguimiento» de la pestaña MONITOREO** (junto a TABLERO), con el mismo patrón con que CONFIGURACION aloja RESPALDO y ENTORNO; **no** es una pestaña propia porque RF-01 fija siete (D-76). Al exigir `monitoreo.ver`, el operador no lo alcanza.
...
**Trazabilidad:** RF-25, RF-05; RN-08; RNF-01, RNF-06, RNF-07, RNF-11, **RNF-08, RNF-12, RNF-13**; D-27, **D-34, D-35, D-40, D-50, D-55, D-76**; H-16, H-23, H-28.
...
- **1b. Acción no permitida para su rol.** Si la sesión es de operador, el sistema no muestra el bloque REPORTES —la pestaña **MONITOREO** que lo aloja le está denegada— y registra el intento. [D-35, RNF-12, **D-76**]
...
8. **Dado** una sesión con rol operador, **Cuando** el operador intenta abrir **MONITOREO** (la pestaña que aloja la sub-pestaña REPORTES), **Entonces** el sistema responde «Acción no permitida para su rol» y no genera ningún archivo. [D-76, D-35, RNF-12]
...
- **Punto de entrada:** igual que CU-19, el bloque **CONCENTRADAS / ESPECIALES** se renderiza en la **sub-pestaña «REPORTES y seguimiento» de la pestaña MONITOREO** (D-76); no es una pestaña propia.
...
**Trazabilidad:** RF-26, RF-29; RN-04, RN-08; RNF-01, RNF-08, RNF-09, **RNF-12, RNF-14, RNF-15**; D-23, D-25, **D-33, D-35, D-41, D-42, D-50, D-55, D-56, D-76**; H-04, H-14.
...
| **D-75** (la col. 30 `cliente_notificado` no se persiste; la cola de CU-09 la resuelve el supervisor mientras no exista la propuesta de D-60; el operador no ve MONITOREO, GRAFICOS ni reportes) | **CU-09**, **CU-01**, **CU-18**, **CU-19** |
| **D-76** (el bloque REPORTES de CU-19 y CU-20 se renderiza en la sub-pestaña «REPORTES y seguimiento» de MONITOREO: no es una pestaña propia) | **CU-19**, **CU-20**, **CU-01**, **CU-18** |
| **D-77** (lanzador de la jornada `GGTO.bat` + `GGTO.ps1`: despliegue del puesto y arranque del servidor; no hace copias de seguridad) | **CU-01**, **CU-21**, **CU-22** |
| **D-78** (los CSV de ingesta viven en `C:\GGTO\datos` y solo se buscan allí: el lanzador lista los `.csv` de esa carpeta y no mira el proyecto, Descargas ni el Escritorio) | **CU-08** |
| **D-79** (la INGESTA lista los `.csv` de la carpeta de datos y lee el elegido desde la página, preseleccionando el del día; el selector del navegador queda como alternativa) | **CU-08** |
